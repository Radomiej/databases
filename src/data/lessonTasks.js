const task = (id, title, prompt, hint, solution, expected) => ({
  ...
  solution,
  expected,
  successMessage: 'Zadanie zaliczone — wynik pasuje do wymagań tego ćwiczenia.',
});

export const ADDITIONAL_LESSON_TASKS = {
  'select-limit': [
    task('authors-list', 'Autorzy z limitem', 'Pokaż imię i nazwisko pierwszych trzech autorów z tabeli autorzy.', 'Wybierz kolumny imie i nazwisko z tabeli autorzy i dodaj LIMIT 3.', 'SELECT imie, nazwisko FROM autorzy LIMIT 3;', { columns: ['imie', 'nazwisko'], rows: [['Adam', 'Mickiewicz'], ['Bolesław', 'Prus'], ['Henryk', 'Sienkiewicz']], strictOrder: true }),
  ...
  where: [
    task('price-filter', 'Filtr ceny', 'Pokaż tytuły i ceny książek kosztujących co najmniej 40.', 'Użyj warunku cena >= 40. Kolejność danych pozostaw taką, jaką zwraca tabela.', 'SELECT tytul, cena FROM ksiazki WHERE cena >= 40;', { columns: ['tytul', 'cena'], rows: [['Bieguni', 44.9], ['Faraon', 41.2]], strictOrder: true }),
    task('author-filter', 'Warunek z AND', 'Pokaż polskich autorów o id nie większym niż 3.', 'Połącz kraj = \'Polska\' z id <= 3. Nie potrzebujesz jeszcze sortowania.', "SELECT imie, nazwisko FROM autorzy WHERE kraj = 'Polska' AND id <= 3;", { columns: ['imie', 'nazwisko'], rows: [['Adam', 'Mickiewicz'], ['Bolesław', 'Prus'], ['Henryk', 'Sienkiewicz']], strictOrder: true }),
  ],
  'order-by': [
    task('oldest-books', 'Najstarsze książki', 'Pokaż cztery najstarsze książki wraz z rokiem wydania.', 'Sortuj po rok_wydania i id rosnąco, a potem ogranicz wynik do 4 rekordów.', 'SELECT tytul, rok_wydania FROM ksiazki ORDER BY rok_wydania, id LIMIT 4;', { columns: ['tytul', 'rok_wydania'], rows: [['Pan Tadeusz', 1834], ['Kordian', 1834], ['Lalka', 1890], ['Quo vadis', 1896]], strictOrder: true }),
    task('recent-loans', 'Najnowsze wypożyczenia', 'Pokaż dwóch czytelników z najnowszymi datami wypożyczenia.', 'Sortuj data_wypozyczenia malejąco i użyj LIMIT 2.', 'SELECT czytelnik, data_wypozyczenia FROM wypozyczenia ORDER BY data_wypozyczenia DESC LIMIT 2;', { columns: ['czytelnik', 'data_wypozyczenia'], rows: [['Anna Nowak', '2026-03-02'], ['Tomasz Zieliński', '2026-02-18']], strictOrder: true }),
  ],
  'like-null': [
    task('selected-years', 'Kilka wartości przez IN', 'Pokaż tytuły książek wydanych w 1890 albo 1900 roku.', 'Użyj IN (1890, 1900). Możesz wykorzystać poznane wcześniej ORDER BY id.', 'SELECT tytul FROM ksiazki WHERE rok_wydania IN (1890, 1900) ORDER BY id;', { columns: ['tytul'], rows: [['Lalka'], ['Krzyżacy']], strictOrder: true }),
  ...
  aggregates: [
    task('line-summary', 'Podsumowanie pozycji', 'Oblicz łączną liczbę sprzedanych sztuk i średnią cenę pozycji zaokrągloną do dwóch miejsc.', 'Użyj SUM(ilosc) oraz ROUND(AVG(cena_sztukowa), 2) w tabeli pozycje_zamowien.', 'SELECT SUM(ilosc) AS suma_sztuk, ROUND(AVG(cena_sztukowa), 2) AS srednia_cena FROM pozycje_zamowien;', { columns: ['suma_sztuk', 'srednia_cena'], rows: [[12, 701.64]], strictOrder: true }),
  ...
  'group-by': [
    ...
    task('customers-by-city', 'Klienci według miasta', 'Policz klientów w każdym mieście, od największej grupy.', 'Pogrupuj po miasto i sortuj po aliasie liczba_klientow malejąco, a potem po mieście.', 'SELECT miasto, COUNT(*) AS liczba_klientow FROM klienci GROUP BY miasto ORDER BY liczba_klientow DESC, miasto;', { columns: ['miasto', 'liczba_klientow'], rows: [['Gdańsk', 2], ['Kraków', 2], ['Warszawa', 1], ['Wrocław', 1]], strictOrder: true }),
  ],
  having: [
    task('popular-products', 'Produkty w wielu sztukach', 'Pokaż produkty, których łączna zamówiona liczba sztuk wynosi co najmniej 2.', 'Pogrupuj po produkt_id i odfiltruj grupy przez HAVING SUM(ilosc) >= 2.', 'SELECT produkt_id, SUM(ilosc) AS sztuk FROM pozycje_zamowien GROUP BY produkt_id HAVING SUM(ilosc) >= 2 ORDER BY produkt_id;', { columns: ['produkt_id', 'sztuk'], rows: [[1, 3], [2, 3], [3, 2], [5, 2]], strictOrder: true }),
  ...
  'inner-join': [
    task('large-quantities', 'Pozycje zamówione wielokrotnie', 'Pokaż nazwy produktów, które występują w pozycji zamówienia z ilością co najmniej 2.', 'Połącz pozycje_zamowien z produktami po produkt_id i użyj WHERE ilosc >= 2.', 'SELECT p.nazwa, z.ilosc FROM pozycje_zamowien z INNER JOIN produkty p ON p.id = z.produkt_id WHERE z.ilosc >= 2 ORDER BY z.id;', { columns: ['nazwa', 'ilosc'], rows: [['Kamera internetowa', 2], ['Mysz bezprzewodowa', 2], ['Klawiatura mechaniczna', 2]], strictOrder: true }),
